...
const CardHero = ({ hero, addFav }: PropsCardHero) => {
  ...
  return (
    <Box cursor="pointer" position={"relative"}>
      ...
      >
        <MotionBox color="red.400" cursor="pointer" h="30px" position={"absolute"} right={5} top={5} w="30px" whileTap={{ scale: 0.6 }} zIndex="2" onClick={() => addFav(hero.id, hero.name, image)}>
        ...
        <NextLink passHref
          ...
          >
            ...
            <Text color="#fff" fontFamily={"primary"} fontSize={"20px"} fontWeight={"semibold"} left={5} position={"absolute"} textShadow="2px 2px #000" top={310} >{hero.name}</Text>
          </Box>
        </NextLink>
      </MotionBox >
    </Box >
