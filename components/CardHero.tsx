...
const CardHero = (hero: ModelHeroData) => {
  return (
    <Box cursor="pointer" position={"relative"}>
      <Box cursor="pointer" h="25px" left={2} position={"absolute"} top={2} w="25px" zIndex="999">
      ...
      <NextLink passHref
        href={`/heroe/${hero.id}`}>
        <Box>
          <ChakraImage src={`${hero.thumbnail.path}.${hero.thumbnail.extension}`} />
          <Text>{hero.name}</Text>
        </Box>
      </NextLink>
    </Box>
