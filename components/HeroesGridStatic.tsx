...
import CardHeroStatic from "./CardHeroStatics";

interface PropsHeroGrid {
  heroesList: ModelDataMarvelGet;
}

const HeroesGridStatic = ({ heroesList }: PropsHeroGrid) => {
      ...
      <SimpleGrid columns={4} minChildWidth="280px" p={8} spacing={"20px"}>
        {heroesList &&
          heroesList?.data?.results.map((hero) => (
            <CardHeroStatic key={hero.id} addFav={addFavorite} hero={hero} />
          ))}
      </SimpleGrid>
    </Box>
  );
...
export default HeroesGridStatic;
