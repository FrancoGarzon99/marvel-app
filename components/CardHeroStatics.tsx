...
interface PropsCardHeroStatic {
  hero: ModelHeroData;
  addFav: (id: number, name: string, image: string) => void;
  activeFn?: boolean;
}

const CardHeroStatic = ({ hero, addFav, activeFn = true }: PropsCardHeroStatic) => {
      ...
      >
        {activeFn && (
          <MotionBox
            ...
            whileTap={{ scale: 0.6 }}
            zIndex="2"
            onClick={() => addFav(hero.id, hero.name, image)}
          >
            <svg
              className="h-5 w-5"
              fill="currentColor"
        ...
        {activeFn ? (
          <NextLink passHref href={`/heroes/${hero.id}`}>
            <Box
              borderRadius={7}
